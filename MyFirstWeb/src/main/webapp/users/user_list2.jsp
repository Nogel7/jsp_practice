...
<%
    ...
    // DB에 있는 자료를 꺼냈을때, 곧바로 java데이터로 받아오는 작업을 수행한 다음
	...
	// ArrayList<UserVO> 자료형을 이용해 한 줄 한 줄 단위로 여러 row를 저장합니다 (Array List는 가변적이다(정해져있지않다))
    ...
    while(rs.next()){// rs.next는 이동한 데이터가 있으면 true 없으면 false
    ...
    //out.println(userList + "<br/>");
    	...
    	// 이 영역에서 바로 .close()를 쓰고도 데이터를 body태그에서 쓰러면
    ...
    */
     ...
     // getAllUserList() 호출하는 것으로 필요 데이터 적재가 끝납니다.
...
<body>
    <!-- 향상된 for문을 이용해서 userList 내부이 데이터를
