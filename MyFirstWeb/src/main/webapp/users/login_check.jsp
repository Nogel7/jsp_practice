...
<%
    ...
    try{
    	...
    	ResultSet rs = pstmt.executeQuery();// SQL쿼리문을 java에서 쓸수 있도록 resultset에 저장
    	*/
    	...
    	// 아이디가 없다면 null이 들어온
    	if(user.getuId() != null){// 현재 -1 받아온 쿼리에 데이터가 있으면 true 없으면 false 
    		...
    		System.out.println("DB내 유저 날린 아이디 : " + uId);
    		System.out.println("DB내 유저 날린 비밀번호 : " + uPw);
    		// 폼에서 받아온 아이디 테이블 내부 아이디, 폼에서 받아온 비밀번호와 테이블 내부 비밀번호 체크
    		if(fId.equals(uId) && fPw.equals(uPw)){// 폼에서 받아온 아이디와 DB에서 받아온 아이디가같을 경우 로그인
