...
<body>
   ...
   <c:if test="${sessionScope.session_id ne null}">
	   <a href="http://localhost:8181/MyFirstWeb/users/insertForm.do"><button>글쓰기</button></a>
    </c:if>
    
     ${dto }
     <!-- document의 pageination 파트를 보고 1부터 10까지 나열해보세요. -->
     <nav aria-label="Page navigation example">
  <ul class="pagination justify-content-center">
  <c:if test="${dto.startPage ne 1}">
    <li class="page-item ${dto.startPage eq 1 ? 'disabled' : '' }"><a class="page-link" href="http://localhost:8181/MyFirstWeb/boardList.do?pageNum=${dto.startPage - 1 }">&laquo;</a></li>
  </c:if>
    ...
    <li class="page-item ${dto.endPage eq dto.totalPages ? 'disabled' : '' }"><a class="page-link" href="http://localhost:8181/MyFirstWeb/boardList.do?pageNum=${dto.endPage + 1 }">&raquo;</a></li>
  </ul>
</nav>
    <hr/>
    <h3>부트스트랩 없이 만들기</h3>
