...
public class ServletCustom extends HttpServlet {
	 ...
	 */
	public void destroy() {
		System.out.println("/banana 접속시 생성된 객체는 서버 정료로 인해 파기됩니다.");
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 목적지 주소는 localhost:프로젝트/프로젝트명/이후경로 <- /이후경로 부터 복붙하면 된다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. request.setAttribute()를 이용해 포워딩 목적지롤 보낼 데이터 바인딩(저장)
		request.setAttribute("jsp", jsp);
		request.setAttribute("boot", boot);
		request.setAttribute("jpa", jpa);
