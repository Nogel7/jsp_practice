...
// 굳이 차이를 두자면 VO는 DB 테이블 형태르르 그대로 전달할 때
...
public class BoardDTO {
	...
	public BoardDTO(int boardCount, int currentPage) {
		...
		if(boardCount == 0) {
			...
			// 게시글을 이용해 전체 페이지 갯수 구하기(totalPages 값 구하기)
			...
			// * 시작, 끝 페이지는 게시글이 아닌 아래 깔리 페이지 버튼이므로 NAV_NUM을 사용합니다.
			int navNum = 0;// 식별번호
			if(currentPage % 10 == 0) { 
			    // 10배수 페이지는 자신이 속한 그룹보다 페이지 삭별숫자가 1 크게 나옴.
